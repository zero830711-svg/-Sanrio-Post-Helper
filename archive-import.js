...
async function db(){return new Promise((res,rej)=>{const r=indexedDB.open('sanrioPostHelperDB',1);r.onsuccess=()=>res(r.result);r.onerror=()=>rej(r.error)})}async function all(){const d=await db();return new Promise((res,rej)=>{const r=d.transaction('popularPosts').objectStore('popularPosts').getAll();r.onsuccess=()=>res(r.result);r.onerror=()=>rej(r.error)})}async function put(x){const d=await db();return new Promise((res,rej)=>{const t=d.transaction('popularPosts','readwrite');t.objectStore('popularPosts').put(x);t.oncomplete=res;t.onerror=()=>rej(t.error)})}
